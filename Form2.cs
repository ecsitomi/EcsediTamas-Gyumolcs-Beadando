...
namespace EcsediTamas_Gyumolcs_Beadando
{
    ...
    {
        string muvelet;
        public Form2(string muvelet, Object param = null)
        {
            InitializeComponent();
        ...
        private void modosit()
        {
            ...
            Program.kezdolap.listBox_Gyumolcs.Items.Add(gyumolcs);
            this.Close();
        }

        void ujGyumolcs()
        {
            int termekkod = Program.kezdolap.listBox_Gyumolcs.Items.Count + 1;
            if (String.IsNullOrEmpty(textBox_Nev.Text))
            {
                MessageBox.Show("Hiányzó név!");
                textBox_Nev.Focus();
